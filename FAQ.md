...
### Ce sunt valorile instantanee?

Valorile instantanee sunt măsurători în timp real de la contorul inteligent (smart meter):

- **Energie activă consumată (EA)** — energia electrică consumată, exprimată în kWh (kilowați-oră)
- **Energie activă produsă (EAP)** — energia electrică produsă (doar prosumer), exprimată în kWh
- **Tensiuni de fază** — tensiunea pe fiecare din cele trei faze (R, S, T), exprimată în V (volți)
- **Curenți de fază** — curentul pe fiecare din cele trei faze (R, S, T), exprimat în A (amperi)
- **Putere activă instantanee** — puterea în momentul citirii, exprimată în kW (kilowați)
- **Energie reactivă** — component inductiv/capacitiv, exprimat în kVArh (kilovolt-amperi reactivi-oră)

Datele instantanee sunt disponibile doar cu automatizări sau la apăsarea manuală a butonului de actualizare. Fără actualizare, datele ar putea fi mai vechi.

---

### Cum actualizez valorile instantanee automat?

Creează o automatizare care apasă butonul „Actualizare valori instantanee" la intervale regulate:
...
**Actualizare la 15 minute (mai frecventă):**
```yaml
automation:
...
Înlocuiește `RO001EXXXXXXXXX` cu codul POD-ului tău. Alege intervalul care ți se potrivește (orar pentru consum normal, 15 minute pentru monitorizare în timp real).

---

### Cum adaug valorile instantanee în Energy Dashboard?

Valorile instantanee sunt senzori compatibili cu Energy Dashboard:

1. Navighează la **Setări** → **Dashboards** → **Energie**
2. Apasă **Configurare** (⚙️) în colțul dreapta-sus
3. În secțiunea **Gaz/Electricitate** → **Consum**, adaugă:
   - `sensor.reteleelectrice_RO001EXXXXXXXXX_valoare_instantanee_consum` (Energy: kWh, TOTAL_INCREASING)
4. În secțiunea **Producție solară**, adaugă (doar prosumer):
   - `sensor.reteleelectrice_RO001EXXXXXXXXX_valoare_instantanee_productie` (Energy: kWh, TOTAL_INCREASING)
5. Salvează — valorile vor apărea în Energy Dashboard

**Important**: Pentru a avea date actualizate în Energy Dashboard, recomandăm să creezi o automatizare care apasă butonul de actualizare la intervale regulate (ex: la fiecare oră sau 15 minute).

---

### Ce este senzorul „Date furnizor"?

Senzorul „Date furnizor" conține informații suplimentare despre furnizor și configurația contorului:

- **Furnizor și PRE** — compania care vinde energia (furnizor) și cea care o distribuie (operator distribuție)
- **CUI furnizor** — codul unic de identificare fiscală al furnizorului
- **Informații client** — nume, adresă, judet
- **Detalii contract** — putere aprobată, putere evacuată (prosumer), punct de delimitare
- **Tensiune și stare** — tensiunea nominală și starea contractului (activ/inactiv)
- **Detalii contor** — seria, marca, tip, data montării, precizie, constantă
- **Date de activare** — când s-a activat furnizorul și consumatorul
- **Certificat CER** — numărul și versiunea certificatului de înregistrare
- **Telecitire** — dacă contorul suportă citire la distanță
- **Informații de distribuție** — zonă, unitate operațională, județ, formulă de agregare

Aceste date sunt disponibile pentru **TOATE POD-urile**, indiferent de tip contract sau smart meter.
...
| Valoare instantanee consum | `total_increasing` | Consum rețea în timp real (smart meter) |
| Valoare instantanee producție | `total_increasing` | Producție solară în timp real (smart meter) |

Navighează la **Setări** → **Dashboards** → **Energie** și adaugă senzorii în secțiunile corespunzătoare. Pentru valori instantanee actualizate în timp real, recomandăm să creezi o automatizare care apasă butonul de actualizare la intervale regulate.
